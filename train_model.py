...
# Set tht MLFlow tracking server uri
uri = "http://127.0.0.1:6001"
# Use mlflow.set_tracking_uri to set the uri
mlflow.set_tracking_uri(uri)

# Set experiment name
email = "sergio.hidalgo@yachaytech.edu.ec"  # Use a customized experiment name
...
with mlflow.start_run(run_name=run_name):
    ...
    model_info = mlflow.sklearn.log_model(
        ...
        artifact_path="model_artifacts",  # Set the artifact path appropriately as a string
        signature=signature,
        input_example=X_train,
        registered_model_name=pipeline.generate_model_name(),  # Optional Replace with a static name if needed - you will need to use the same static name when updating to a newer version of the model
    )
